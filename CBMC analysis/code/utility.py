...
def Grover(oracle_index ,nqubits,iterations,backend):
    ...
    job = backend.run(transpiled_grover_circuit,shots=32)
...
def BSS_BIC(Input):
    ...
    p = int(np.log2(len(BICs)))
    m = sum(BICs<10**40)
    selected_index = np.argmin(BICs)
    Selected_Scores = BICs[selected_index]
    return([selected_index,Selected_Scores])
