...
foreach(RNA_name=RNA_namelist) %dopar% {
  library(Seurat)
# ttt = ttt + 1; print(ttt); print(Sys.time() - t1)
# dat0 = read.csv("../data/NK_prof_160000.csv")
...
for(case_sim in 1: num_sims){
  ...
  for(trun_num in 1:(p-1)){
    ...
    # acc_mat = read.csv(paste0("../data/result_BIC/results_", RNA_name, "/num", trun_num, "_test_accu.csv"))
    FP_mat  = FP_mat_list[[trun_num]]
    FN_mat  = FN_mat_list[[trun_num]]
    AUC_mat = AUC_mat_list[[trun_num]]
    
    # MSE_res_array[case_sim, method_id, trun_num] = acc_mat[case_sim,result_name_colname]
  ...
  for(DE_method in DE_method_list){
    ...
    for(trun_num in 1:(p-1)){
      ...
      # acc_mat = read.csv(paste0("../data/result_BIC/results_", RNA_name, "/num", num_vars_to_use,"_test_accu.csv"))
...
# load(paste0("../results/Jun8/AUC_res_array ", RNA_name,".rda"))

# save.image(file = paste0("all_objects_real2_", RNA_name, ".rda"))
stopCluster(cl)
...
dim(current_array)
  ...
  yrange = c(0.9,1)#range(mean_array[1,x_range])
